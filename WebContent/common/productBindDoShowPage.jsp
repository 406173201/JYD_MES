...
<script id="productBindDoShowPageTemplate" type="x-tmpl-mustache">
...
<script type="text/javascript">
	...
	var productBindDoShowPageTemplate = $("#productBindDoShowPageTemplate").html();
	//02将页面变量内容交给mustache库,使用mustache技术加载指定模板
	Mustache.parse(productBindDoShowPageTemplate);
	//渲染分页的内容
	//url total pageNo pageSize currentSize idElement:将paginatetemplate加载到哪一个页面版块 预留一个回调函数
	function renderPageProductBindDoShow(url, total, pageNo, pageSize, currentSize, idElement,
			callback) {
		//得到最大的页码,使用向上取整的函数
		...
		$("#" + idElement).html(Mustache.render(productBindDoShowPageTemplate, view));
